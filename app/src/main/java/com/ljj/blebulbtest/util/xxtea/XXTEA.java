...
package com.ljj.blebulbtest.util.xxtea;
...
public final class XXTEA {
    ...
    public static final byte[] encrypt(byte[] data, byte[] key,boolean len) {
        if (data.length == 0) {
            return data;
        }
        return toByteArray(
                encrypt(toIntArray(data, len), toIntArray(fixKey(key), false)), false);
    }
    ...
    public static final byte[] decrypt(byte[] data, byte[] key,boolean len) {
        if (data.length == 0) {
            return data;
        }
        return toByteArray(
                decrypt(toIntArray(data, len), toIntArray(fixKey(key), len)), len);
    }
